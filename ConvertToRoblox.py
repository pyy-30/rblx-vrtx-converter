...
Converts a Vortex `.vrtx` file to a Roblox `.rbxlx` file in two steps:
    1. VRTX_to_JSON.py   (vrtx → json)
    2. JSON_to_RBXLX.py  (json → rbxlx)
...
Usage:
    py ConvertToRoblox.py [filename.vrtx] [output.rbxlx] [--verbose] [--keep-json]

If no filename is given, the script lists every `.vrtx` in the first
...
def find_converters_dir():
    for c in CONVERTER_CANDIDATES:
        if (c / "VRTX_to_JSON.py").exists() and (c / "JSON_to_RBXLX.py").exists():
            return c
    return None
...
def main():
    ...
    if converters_dir is None:
        ...
        for c in CONVERTER_CANDIDATES:
            print(f"  {c}")
        print("Expected to find VRTX_to_JSON.py and JSON_to_RBXLX.py there.")
        sys.exit(1)
    ...
    if args:
        ...
        if resolved.exists():
            ...
            # Bare filename → look inside the TargetFiles folders
            found = None
            for c in TARGET_CANDIDATES:
        ...
        in_path = pick_target(targets_dir, ".vrtx")
    ...
    try:
        run([sys.executable, str(converters_dir / "VRTX_to_JSON.py"), str(in_path)], verbose)
    except subprocess.CalledProcessError as e:
        print("VRTX_to_JSON.py failed:")
        if e.stderr:
    ...
    try:
        run([sys.executable, str(converters_dir / "JSON_to_RBXLX.py"),
             str(json_path), str(out_path)], verbose)
    except subprocess.CalledProcessError as e:
        print("JSON_to_RBXLX.py failed:")
        if e.stderr:
            print(e.stderr.decode(errors="replace"))
        sys.exit(1)
